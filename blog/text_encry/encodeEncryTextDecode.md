...
> 本文首先了解编码的发展，理解UTF-8的编码方式，然后使用Python文件操作，对非英文文本文件进行自定义打乱，使得文件不能正常显示达到加密的效果

## 编码发展

### ASCII

最先出现的编码方式，ASCii码表，当然是诞生在最发达的美国，该编码表是单字节编码表，非常简单
...
从b'0000_0000'(0)~b'0111_1111'(127)，涵盖了数字，字母和各种特殊符号，比如`space` 是`40` 。值得一提的是，Ascii码只用了7位，还少用了一位，*火星救援* 的男主就是靠Ascii码传递信息回家的哦。

### Unicode

英文编码（如Ascii码表）只需要7位就可以，但是对于欧洲的各种字母，对于亚洲的方块字只使用7位是不可能的，世界上各个国家的语言都有各自的编码，同样的文件，使用不同的解码方式就会造成乱码的现象，所以需要一个全球统一的编码，把所有的符号都囊入其中，这就是Unicode的来源。

Unicode对每一个字符都有一个独一无二的编码，但是他只规定的单个字符的编号，却没有规定存储规则。
举例来说，英文字母只需要7位就可以存储，但是汉字就需要两个字节，那么当他们同时出现在文件中，我怎么知道应该取一个字节读取英文字母还是应该取两个字节读取汉字呢？

UTF-8就是该问题的一个解决方法，并且使用的最为广泛。

### UTF-8

UTF-8编码解释如下，第一个字节的联系前`n`位为1，第`n+1`位为0，那么就说明该字符的内容有`n`个字节，之后的每个字节的前两位都是`10` 
...
如上表所示，首先拿到该字符的Unicode编码，然后根据左侧一列，对照出对应的UTF-8的编码，举个栗子，`吃`的Unicode编码为`&#x5403;_0101_0100_0000_0011`，在表中是左侧第三行的位置，所以按照右侧第三列往x插入得到`1110_0101_1001_0000_1000_0011` 即`E59083` 
